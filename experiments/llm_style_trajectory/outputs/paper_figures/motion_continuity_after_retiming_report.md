# Motion Continuity Dry-Run Report

Scope: motion continuity dry-run only; not real robot dynamics, not IK, not joint-space velocity/acceleration/torque checking, and not real robot control.

- input: `experiments\llm_style_trajectory\outputs\batch_20260613_154131\u5c71_xingkai_20260613_154132_009898\robot_target_poses_smoothed.csv`
- input_kind: `target_pose`
- time_source: `provided`

## Summary

| Field | Value |
|---|---|
| `point_count` | `271` |
| `duration_s` | `22.039876274` |
| `path_length_m` | `0.359530547` |
| `dt_min_s` | `0.059084524` |
| `dt_max_s` | `0.194349397` |
| `dt_mean_s` | `0.081629171` |
| `dt_nonpositive_count` | `0` |
| `max_step_3d_m` | `0.002487672` |
| `max_speed_m_s` | `0.01792` |
| `max_speed_jump_m_s` | `0.01792` |
| `max_accel_m_s2` | `0.274132141` |
| `max_jerk_m_s3` | `4.193553547` |
| `jerk_peak_count` | `0` |
| `quaternion_norm_min` | `1.0` |
| `quaternion_norm_max` | `1.0` |
| `quaternion_normalized` | `True` |
| `orientation_fixed_or_smooth` | `True` |
| `segment_counts` | `{"connector": 36, "stroke": 235}` |
| `segment_stats` | `{"connector": {"max_accel_m_s2": 0.027066832, "max_jerk_m_s3": 0.143088626, "max_speed_m_s": 0.0128, "point_count": 36}, "stroke": {"max_accel_m_s2": 0.274132141, "max_jerk_m_s3": 4.193553547, "max_speed_m_s": 0.01792, "point_count": 235}}` |
| `recommended_for_coppeliasim_playback` | `True` |
| `recommended_for_ik_dry_run` | `True` |
| `warnings` | `[]` |
| `failure_reasons` | `[]` |

## Boundary

These thresholds are conservative dry-run gates for simulation and future IK preparation. They are not claimed to be real AUBO i5 controller limits. This report does not check joint-space velocity, joint acceleration, torque, collision, singularity, or true reachability.
